...
/**
 * Aurora — aurora boreal procedural (WebGL2 + ogl), adaptada de
 * React Bits (github.com/DavidHDev/react-bits, MIT + Commons Clause).
 *
 * Re-tematizada para o AnimesIce: em vez do roxo/verde genérico, uma faixa
 * de gelo (ice) atravessando o céu noturno — o "sinal da madrugada" no
 * próprio fundo. Uso: camada absoluta atrás do conteúdo do hero.
 *
 * Acessibilidade/robustez:
 *  - não renderiza com prefers-reduced-motion;
 *  - não renderiza sem WebGL2 (fallback = gradiente CSS já existente);
 *  - limpa o contexto WebGL no unmount.
 */
...
export function Aurora({
  ...
  /** 3 paradas de cor — o rampe da aurora, da esquerda para a direita. */
  colorStops?: string[];
  amplitude?: number;
  blend?: number;
  speed?: number;
  ...
  useEffect(() => {
    ...
    // WebGL2 obrigatório (shader #version 300 es) — sem ele, nada acontece
    // e o gradiente CSS do hero permanece como fallback.
    try {
      const testCanvas = document.createElement("canvas");
      const testGl = testCanvas.getContext("webgl2");
      if (!testGl) return;
      // Libera o contexto de teste imediatamente.
