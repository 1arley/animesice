...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    href: "/admin/usuarios",
    label: "Usuários",
    icon: "M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4zm6 0a3 3 0 10-3-3 3 3 0 003 3z",
  },
  {
    href: "/admin/moderacao",
    label: "Moderação",
    icon: "M9 12l2 2 4-4m5.618 4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.016L2 12l1.382 6.04A11.955 11.955 0 0012 21.056a11.955 11.955 0 008.618-3.016L22 12l-1.382-5.984z",
  },
  {
    href: "/admin/posts",
    label: "Posts",
  ...
  },
  {
    href: "/admin/blog",
    label: "Blog",
    icon: "M4 5.5A2.5 2.5 0 016.5 3H12v16H6.5A2.5 2.5 0 004 21.5v-16zM20 5.5A2.5 2.5 0 0017.5 3H12v16h5.5a2.5 2.5 0 012.5 2.5v-16z",
  },
  {
    href: "/admin/pedidos",
    label: "Pedidos",
    icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M7 16h.01",
  },
  {
...
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto max-w-shelf px-4 py-4 md:py-6">
      <div className="flex flex-col gap-4 md:flex-row md:gap-6">
        <AdminSidebar pathname={pathname} mobileNavOpen={mobileNavOpen} setMobileNavOpen={setMobileNavOpen} isSuperadmin={isSuperadmin} />
...
function AdminSidebar({ pathname, mobileNavOpen, setMobileNavOpen, isSuperadmin }: { pathname: string; mobileNavOpen: boolean; setMobileNavOpen: (v: boolean) => void; isSuperadmin: boolean }) {
  const items = NAV_ITEMS.filter((item) => !item.superadminOnly || isSuperadmin);

  return (
    <>
      <button className="admin-tab sticky top-2 z-30 flex w-full items-center justify-between gap-2 bg-ink/95 backdrop-blur md:hidden" onClick={() => setMobileNavOpen(!mobileNavOpen)} aria-expanded={mobileNavOpen} aria-controls="admin-navigation">
        ...
        <span className="max-w-40 truncate text-ice">{items.find((item) => (item.exact ? pathname === item.href : pathname.startsWith(item.href)))?.label ?? "Admin"}</span>
      </button>
        ...
        <nav className="border border-hairline bg-panel p-3 md:sticky md:top-4">
          ...
          <div className="space-y-0.5">
            {items.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setMobileNavOpen(false)} className={`admin-nav-item ${active ? "admin-nav-item-active" : ""}`}>
                  <svg className="h-4 w-4 flex-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
                  </svg>
                  <span>{item.label}</span>
                  {item.superadminOnly && <span className="ml-auto badge-dot bg-signal" />}
                </Link>
              );
            })}
          </div>
          <div className="mt-3 border-t border-hairline pt-3">
            <Link href="/" onClick={() => setMobileNavOpen(false)} className="admin-nav-item">
              <svg className="h-4 w-4 flex-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
